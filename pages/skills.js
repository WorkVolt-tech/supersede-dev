...
export async function mountSkills(__mountOptions = {}) {
  ...
  // ── Migrate / normalize battle_skills ───────────────────────────
  // Earlier builds could write arrays of length 7+ (ghost passive bug).
  // Rescue any non-null entries at the wrong indices so the player keeps
  // their slotted skills, then truncate to the canonical 6-slot layout
  // (A1..A4 at 0..3, P1..P2 at 4..5).
  ;(() => {
    const ACTIVE_SLOTS  = 4
    const PASSIVE_SLOTS = 2
    const TOTAL = ACTIVE_SLOTS + PASSIVE_SLOTS
    const src = [...battleSkills]
    const normalized = new Array(TOTAL).fill(null)
    const overflow = []
    for (let i = 0; i < src.length; i++) {
      const sid = src[i]
      if (!sid) continue
      if (i < TOTAL) normalized[i] = sid
      else overflow.push(sid)
    }
    // Place overflow into first empty matching slot by skill type
    overflow.forEach(sid => {
      const nd = NODES.find(n => n.id === sid)
      if (!nd) return
      const isPassive = nd.battleType === 'passive'
      if (isPassive) {
        for (let i = ACTIVE_SLOTS; i < TOTAL; i++) {
          if (!normalized[i]) { normalized[i] = sid; return }
        }
      } else {
      ...
      console.warn('[skills] dropped overflow skill (no slot):', sid)
    })
    // Only flag dirty if the on-disk array was actually different
    const same = src.length === normalized.length && src.every((v, i) => v === normalized[i])
    battleSkills = normalized
    if (!same) console.warn('[skills] battle_skills normalized; save to persist.')
  })()
  ...
  // Escape key closes the node-detail panel too (safety net so users are never trapped).
  if (!window._elDetailEscWired) {
    window._elDetailEscWired = true
  ...
  function buildTree() {
    ...
    // Connector lines are drawn by refreshLines() — initial render with no
    // selection produces an empty SVG. Lines appear when the user selects a
  ...
  function refreshLines() {
    ...
    // (Selecting an unlocked-but-available node no longer lights up the
    // back-path. The visual focus comes from a rotating dotted ring
    // around the selected node itself — see .sn.selected.available in
    // the stylesheet.)
    ...
    nodes.forEach(nd => {
      nd.requires.forEach(req => {
        ...
        const col = lit
          ...
          : 'rgba(180,140,60,.35)'
        const sw  = lit ? 2.2 : 1.2
        // All connector lines are dotted — gives the tree a "blueprint"
        // feel and avoids any solid line ever showing.
        lines += `<line x1="${par.x}" y1="${par.y}" x2="${nd.x}" y2="${nd.y}" stroke="${col}" stroke-width="${sw}" stroke-linecap="round" stroke-dasharray="3 5"/>`
      })
    })
    svg.innerHTML = lines
  }
      ...
      body = `
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;flex-wrap:wrap;margin-bottom:.4rem">
          ...
          <div style="display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end">
            ${canUnlock(nd) ? `<button onclick="doUnlock('${nd.id}')" style="font-family:'Cinzel',serif;font-size:.7rem;color:#1a1208;background:rgba(200,184,128,.85);border:1px solid rgba(200,184,128,.9);border-radius:3px;padding:.3rem .8rem;cursor:pointer">✦ Unlock (${nd.cost} SP)</button>` : ''}
            ...
            ${!isUnlocked(nd) && pMet && unmetReqs.length > 0 && metVia.length > 0 ? `<div style="font-family:'Share Tech Mono',monospace;font-size:.46rem;color:#9a8a6a;margin-top:3px">✓ Reachable via ${metVia.map(r=>{ const n=NODES.find(x=>x.id===r); return n?.label||r }).join(', ')}</div>` : ''}
            ${isUnlocked(nd) && canEquip && !inBattle && !battleFull
              ? `<button onclick="stageAddToBattle('${nd.id}')" style="font-family:'Cinzel',serif;font-size:.7rem;color:#cfe8ff;background:rgba(94,174,224,.3);border:1px solid rgba(94,174,224,.6);border-radius:3px;padding:.3rem .8rem;cursor:pointer">+ Add to Battle</button>` : ''}
  ...
  window.stageAddToBattle = (id) => {
    ...
    // Pad array to ACTIVE_SLOTS + PASSIVE_SLOTS so slot indices are stable
    // (A1..A4 at 0..3, P1..P2 at 4..5). Without this, removing a passive
    // would trim trailing nulls and the next passive add would `push` to
    // a phantom index past the slot UI — leaving a ghost passive that the
    // UI can't show but the count thinks is there. Padding both halves
    // keeps the slot indices fixed and makes both branches use the same
    // findIndex pattern.
    const newSkills = [...pendingBattleSkills]
    while (newSkills.length < ACTIVE_SLOTS + PASSIVE_SLOTS) newSkills.push(null)
    // Defensive: drop any junk past the legal slot count (would only
    // happen on a save written by an earlier buggy build).
    if (newSkills.length > ACTIVE_SLOTS + PASSIVE_SLOTS) newSkills.length = ACTIVE_SLOTS + PASSIVE_SLOTS

    if (isPassive) {
      // Passive → first empty P-slot (index 4 or 5). No more push().
      let firstEmpty = -1
      for (let i = ACTIVE_SLOTS; i < ACTIVE_SLOTS + PASSIVE_SLOTS; i++) {
        if (!newSkills[i]) { firstEmpty = i; break }
      }
      if (firstEmpty === -1) { window.showToast('Passive slots full — P1 and P2 are taken'); return }
      newSkills[firstEmpty] = id
    } else {
      // Active → first empty A-slot (index 0–3)
  ...
  window.stageRemoveFromBattle = (id) => {
    ...
    // Keep the array at the canonical slot count so empty slots stay
    // addressable by index. The old "trim trailing nulls" caused a ghost
    // passive bug — when P1 was empty and P2 was nulled, the trim shrank
    // the array, and the next add would push past the visible P-slot.
    while (pendingBattleSkills.length < ACTIVE_SLOTS + PASSIVE_SLOTS) pendingBattleSkills.push(null)
    if (pendingBattleSkills.length > ACTIVE_SLOTS + PASSIVE_SLOTS) pendingBattleSkills.length = ACTIVE_SLOTS + PASSIVE_SLOTS
    markSlotsDirty()
    renderSlotsBar()
    renderBattleSlotsPage()
    const nd = NODES.find(n => n.id === id)
  ...
  function renderSlotsBar() {
    ...
    function slotLabel(nd, id, slotType, slotIdx, isPassive) {
      ...
      return `<span style="font-family:'IM Fell English',serif;font-style:italic;font-size:.7rem;color:#9a8a6a;flex:1">Empty</span>
    ...
    html += `<div style="margin-top:.6rem;display:flex;gap:5px">
      <button onclick="confirmSaveSlots()" style="flex:1;font-family:'Cinzel',serif;font-size:.72rem;color:#1a1208;background:${slotsDirty?'rgba(200,184,128,.85)':'rgba(200,184,128,.5)'};border:1px solid rgba(200,184,128,.8);border-radius:3px;padding:.4rem;cursor:pointer;transition:all .2s">
  ...
  const ELEMENTS = {
    ...
    shadow:{label:'Umbra',color:'#9a6fd8',glow:'#9a6fd888',bg:'rgba(154,111,216,.08)'},
    poison:{label:'Venin',color:'#7aad30',glow:'#7aad3088',bg:'rgba(122,173,48,.08)'},
    arcane:{label:'Lux',color:'#b06eff',glow:'#b06eff88',bg:'rgba(176,110,255,.08)'},
  ...
  const ELEMENTAL_SKILLS = {
    ...
    earth:[{key:'earth_passive_damage_reduction',type:'passive',label:'Stone Skin',desc:'Passive: Reduce all incoming damage by 5.',cost:1,requires:null},{key:'earth_skill_rock_armor',type:'active',label:'Rock Armor',desc:'Active: +20 DEF for 2 turns.',cost:2,requires:'earth_passive_damage_reduction'},{key:'earth_ultimate_earthquake',type:'ultimate',label:'Earthquake',desc:'Ultimate: Massive damage, stuns 1 turn.',cost:3,requires:'earth_skill_rock_armor'},{key:'earth_skill_stone_throw',type:'active',label:'Stone Throw',desc:'Active: 26 earth damage, +bonus from DEF.',cost:2,requires:'earth_passive_damage_reduction'}],
    ...
    metal:[{key:'metal_passive_reflect',type:'passive',label:'Reflect',desc:'Passive: Reflect 10% of damage taken.',cost:1,requires:null},{key:'metal_skill_blade_form',type:'active',label:'Blade Form',desc:'Active: ATK +20, SPD +5 for 2 turns.',cost:2,requires:'metal_passive_reflect'},{key:'metal_ultimate_iron_domain',type:'ultimate',label:'Iron Domain',desc:'Ultimate: ATK doubles for 3 turns.',cost:3,requires:'metal_skill_blade_form'},{key:'metal_skill_blade_arc',type:'active',label:'Blade Arc',desc:'Active: 28 metal damage, ignores 25% DEF.',cost:2,requires:'metal_passive_reflect'}],
    plant:[{key:'plant_passive_heal_still',type:'passive',label:'Regrowth',desc:'Passive: Restore 3 HP/turn when not hit.',cost:1,requires:null},{key:'plant_skill_root_trap',type:'active',label:'Root Trap',desc:'Active: Enemy skips next attack.',cost:2,requires:'plant_passive_heal_still'},{key:'plant_ultimate_nature_overgrowth',type:'ultimate',label:'Overgrowth',desc:'Ultimate: Heal 40 HP and deal 30 damage.',cost:3,requires:'plant_skill_root_trap'},{key:'plant_skill_thorn_volley',type:'active',label:'Thorn Volley',desc:'Active: 26 plant damage + lifesteal.',cost:2,requires:'plant_passive_heal_still'}],
    water:[{key:'water_passive_regen',type:'passive',label:'Flow',desc:'Passive: Recover 5 HP per turn.',cost:1,requires:null},{key:'water_skill_water_shield',type:'active',label:'Water Shield',desc:'Active: Absorb next 30 damage.',cost:2,requires:'water_passive_regen'},{key:'water_ultimate_tsunami',type:'ultimate',label:'Tsunami',desc:'Ultimate: 50 damage, skip enemy turn.',cost:3,requires:'water_skill_water_shield'},{key:'water_skill_water_jet',type:'active',label:'Water Jet',desc:'Active: 28 water damage, pierces shields.',cost:2,requires:'water_passive_regen'}],
    shadow:[{key:'shadow_skill_shadow_lance',type:'active',label:'Shadow Lance',desc:'Active: 28 shadow damage from concealment.',cost:2,requires:null}],
    poison:[{key:'poison_skill_venom_spit',type:'active',label:'Venom Spit',desc:'Active: 24 poison damage + applies Long Decay.',cost:2,requires:null}],
    arcane:[{key:'arcane_skill_resonant_bolt',type:'active',label:'Resonant Bolt',desc:'Active: 30 Lux damage; can echo via Reverberation.',cost:2,requires:null},{key:'arcane_ultimate_standing_chord',type:'ultimate',label:'Standing Chord',desc:'Ultimate: 45 Lux damage that echoes harder.',cost:3,requires:'arcane_skill_resonant_bolt'}],
  ...
  // ─────────────────────────────────────────────────────────────────
  // ELEMENTS TREE (Pass A.2) — new UI replacing old ESP overlay.
  // Lives alongside the old ESP code so we don't break it while testing.
  // The "Elemental Tree" trigger button now calls openElementsTree() (the
  // markup change wired this). Old #esp-overlay code is unreachable.
  // ─────────────────────────────────────────────────────────────────
  let activeElementKey = null  // which element tab is currently shown
  let selectedElNodeId = null  // which node is selected for the detail panel

  function getActiveElementTree() {
    return activeElementKey ? ELEMENT_TREES[activeElementKey] : null
  }
  function getElementCostBadgeText(cost) {
    return cost + '◈'  // ◈ = shard symbol
  }
  function isElementNodeUnlocked(nodeId) {
    return elUnlocked.includes(nodeId)
  }
  function isElementNodeAvailable(node) {
    if (isElementNodeUnlocked(node.id)) return false
    // Roots always available
    if (!node.requires || node.requires.length === 0) return true
    // All requirements must be unlocked
    return node.requires.every(r => isElementNodeUnlocked(r))
  }

  window.openElementsTree = function() {
    document.getElementById('elements-overlay').classList.add('open')
    renderElementsBadge()
    renderElementsTabs()
    // Default to first unlocked element, or Ignis if none unlocked yet
    if (!activeElementKey) {
      const firstUnlocked = Object.keys(ELEMENT_TREES).find(k => isElementUnlocked(k, player))
      activeElementKey = firstUnlocked || 'fire'
    }
    selectedElNodeId = null
    renderElementsTree()
  }
  window.closeElementsTree = function() {
    document.getElementById('elements-overlay').classList.remove('open')
  }

  function renderElementsBadge() {
    document.getElementById('elements-badge').textContent = (shards||0) + ' SHARDS'
  }

  function renderElementsTabs() {
    const list = document.getElementById('elements-tab-list')
    if (!list) return
    list.innerHTML = Object.keys(ELEMENT_TREES).map(elKey => {
      const t = ELEMENT_TREES[elKey]
      const unlocked = isElementUnlocked(elKey, player)
      const hasNodes = t.nodes.length > 0
      const isActive = elKey === activeElementKey
      const locked   = !unlocked || !hasNodes
      const sub = !unlocked ? 'Locked — defeat boss'
                : !hasNodes ? 'Coming soon'
                : (t.nodes.length + ' nodes')
      return '<div class="el-tab ' + (locked?'locked ':'') + (isActive?'active':'') +
             '" data-key="' + elKey + '">' +
             '<span class="el-tab-sigil">' + t.sigil + '</span>' +
             '<span class="el-tab-text">' +
               '<span class="el-tab-name">' + t.label + '</span>' +
               '<span class="el-tab-sub">' + sub + '</span>' +
             '</span></div>'
    }).join('')
    // Bind clicks
    list.querySelectorAll('.el-tab:not(.locked)').forEach(el => {
      el.addEventListener('click', () => {
        activeElementKey = el.dataset.key
        selectedElNodeId = null
        renderElementsTabs()
        renderElementsTree()
      })
  ...
  function renderElementsTree() {
    ...
    const nodesC = document.getElementById('elements-tree-nodes')
    const svg    = document.getElementById('elements-tree-svg')
    const detail = document.getElementById('elements-tree-detail')
    if (!tree || !tree.nodes || tree.nodes.length === 0) {
      header.innerHTML = '<p class="el-header-title">No tree available</p>'
      nodesC.innerHTML = ''
      svg.innerHTML = ''
      detail.innerHTML = ''
      return
    ...
    // Header
    header.innerHTML =
      '<p class="el-header-title">' + tree.sigil + ' ' + tree.label + '</p>' +
      '<p class="el-header-signature">Signature: ' + tree.signatureLabel + '</p>' +
      '<p class="el-header-blurb">' +
        (player.element === activeElementKey
    ...
    if (_elSkills.length) {
      ...
      panel += '<p style="font-family:\'Cinzel\',serif;font-size:.7rem;color:'+tree.color+' !important;margin:0 0 6px">⚔ '+tree.label+' Attack Skills <span style="color:#c9b88f;font-size:.6rem">('+_nUnlocked+'/19 nodes)</span></p>'
      for (let i = 0; i < 3; i++) {
        ...
        let btn
        if (!unlocked) {
          btn = '<span style="font-size:.6rem;color:#9a8a6a">🔒 unlock at '+need+' nodes</span>'
        } else if (isEquipped) {
          btn = '<span style="font-size:.62rem;color:'+tree.color+' !important;font-weight:bold">✓ EQUIPPED</span>'
        } else {
        ...
        panel += '<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0;'+(unlocked?'':'opacity:.55;')+'">'
              +  '<span style="font-size:.66rem;color:#e8dcc0"><b style="color:'+tree.color+' !important">'+tierName+'.</b> '+meta.label+'</span>'
              +  btn + '</div>'
      }
      panel += '<p style="font-size:.55rem;color:#9a8a6a;margin:6px 0 0">Only one elemental attack skill can be equipped at a time.</p>'
      panel += '</div>'
    ...
    tree.nodes.forEach(nd => {
      ...
      nd.requires.forEach(reqId => {
        const par = tree.nodes.find(n => n.id === reqId)
        if (!par) return
    ...
    // Render nodes
    nodesC.style.minWidth  = (PAD_X*2) + 'px'
    nodesC.style.minHeight = (PAD_Y*2) + 'px'
    nodesC.innerHTML = tree.nodes.map(nd => {
      const x = nd.x + PAD_X, y = nd.y + PAD_Y
      const sizeCls = nd.type === 'start' ? 'el-node-st'
                    : nd.type === 'keystone' ? 'el-node-ks'
                    : nd.type === 'notable' ? 'el-node-md'
                    : 'el-node-sm'
      const unlocked = isElementNodeUnlocked(nd.id)
      const avail    = isElementNodeAvailable(nd)
      const stateCls = unlocked ? 'unlocked' : avail ? 'available' : 'locked'
      const selCls   = nd.id === selectedElNodeId ? ' selected' : ''
      const color    = unlocked || avail ? tree.color : '#5a4a3a'
      const costBadge = (unlocked || nd.cost === 0)
        ? ''
        : '<span class="el-node-cost">' + getElementCostBadgeText(nd.cost) + '</span>'
      return '<div class="el-node ' + sizeCls + ' ' + stateCls + selCls +
             '" data-id="' + nd.id + '"' +
             ' style="left:' + x + 'px; top:' + y + 'px">' +
             '<div class="el-node-ring" style="border-color:' + color + '">' +
               costBadge +
             '</div>' +
             '<div class="el-node-lbl">' + nd.label + '</div>' +
             '</div>'
    }).join('')
    // Wire node clicks
    nodesC.querySelectorAll('.el-node').forEach(el => {
      el.addEventListener('click', () => {
        const id = el.dataset.id
        if (selectedElNodeId === id) {
          selectedElNodeId = null
        } else {
          selectedElNodeId = id
        }
    ...
    // Detail panel
    renderElementsTreeDetail()
  }

  function renderElementsTreeDetail() {
    const detail = document.getElementById('elements-tree-detail')
    const tree = getActiveElementTree()
    if (!tree || !selectedElNodeId) {
      detail.innerHTML = '<p style="font-family:\'Share Tech Mono\',monospace;font-size:.6rem;color:#a89e80;letter-spacing:.05em">Select a node to see details.</p>'
      return
    }
    const nd = tree.nodes.find(n => n.id === selectedElNodeId)
    if (!nd) { detail.innerHTML = ''; return }
    const unlocked = isElementNodeUnlocked(nd.id)
    const avail    = isElementNodeAvailable(nd)
    const canAfford = shards >= (nd.cost || 0)
    let actionHtml = ''
    if (unlocked) {
      actionHtml = '<p class="el-pass2-note">⚙ UNLOCKED. Combat effect activates in Phase 2 wiring.</p>'
    } else if (avail && nd.cost > 0) {
      actionHtml =
        '<div class="el-detail-actions">' +
          '<button class="el-buy-btn' + (canAfford?'':' disabled') +
          '" data-buy="' + nd.id + '">Unlock for ' + nd.cost + ' shards</button>' +
        '</div>' +
        '<p class="el-pass2-note">⚙ Combat effect activates in Phase 2 (next session). Spend now to lock in your build.</p>'
    } else if (avail && nd.cost === 0) {
      actionHtml = '<div class="el-detail-actions"><button class="el-buy-btn" data-buy="' + nd.id + '">Activate root</button></div>'
    } else {
      actionHtml = '<p class="el-pass2-note">Requires earlier nodes in this path.</p>'
    }
    detail.innerHTML =
      '<p class="el-detail-title">' + nd.label +
        '<span style="font-family:\'Share Tech Mono\',monospace;font-size:.55rem;color:#a89e80;margin-left:.5rem">' +
        (nd.type||'').toUpperCase() + (nd.battleType?' · '+nd.battleType.toUpperCase():'') +
        '</span></p>' +
      '<p class="el-detail-desc">' + (nd.desc||'') + '</p>' +
      actionHtml
    // Wire buy button
    const btn = detail.querySelector('.el-buy-btn:not(.disabled)')
    if (btn) {
      btn.addEventListener('click', () => buyElementNode(btn.dataset.buy))
    }
  }

  async function buyElementNode(nodeId) {
    const tree = getActiveElementTree()
    if (!tree) return
    const nd = tree.nodes.find(n => n.id === nodeId)
    if (!nd) return
    if (isElementNodeUnlocked(nodeId)) return
    if (!isElementNodeAvailable(nd)) { window.showToast('Locked — earlier nodes required'); return }
    if (shards < (nd.cost||0)) { window.showToast('Not enough Resonance Shards'); return }
    // Spend
    shards -= (nd.cost||0)
    elUnlocked = [...elUnlocked, nodeId]
    player.resonance_shards = shards
    player.elemental_unlocked = elUnlocked
    // Persist
    try {
      const { error } = await supabase.from('players')
        .update({ resonance_shards: shards, elemental_unlocked: elUnlocked })
        .eq('id', player.id)
      if (error) throw error
    } catch (e) {
      console.error('[elements] save failed', e)
      window.showToast('Save failed — try again')
      // Roll back
      shards += (nd.cost||0)
      elUnlocked = elUnlocked.filter(x => x !== nodeId)
      player.resonance_shards = shards
      player.elemental_unlocked = elUnlocked
    }
    // Refresh aliases for legacy code
    espBalance = shards
    renderElementsBadge()
    renderElementsTree()
    // Also update the left-page sidebar shard display
    const espDisplay = document.getElementById('esp-display')
    if (espDisplay) espDisplay.textContent = shards
  }
