...
export async function mountLobby(__mountOptions = {}) {
  ...
  // BC was a local 4-tier badge → {label, color} map. Now sources from
  // reputation.js so the canonical lookup is in one place. Kept as a Proxy
  // so all `BC[player.badge]` callsites work unchanged.
  const BC = new Proxy({}, { get: (_, key) => getBadgeMeta(key) })
  const SC = {
  ...
  // Chapter filter intentionally NOT applied — the lobby is cross-chapter
  // so players in different chapters can still see and challenge each
  // other. (Each card already shows the target's chapter via presence.)
  async function load() {
    const since = new Date(Date.now() - 5 * 60 * 1000).toISOString()
    const { data } = await supabase.from('presence')
      .select('*').neq('player_id', player.id)
  ...
  async function endPvpCombat(won, fromRemote) {
    ...
    try {
      ...
      if (won) {
        ...
        const newBadge = calcBadge(player.moral_score, newKills, player.helps_given)
  ...
  // No chapter filter — cross-chapter lobby (matches load() above).
  supabase.channel('lobby:all')
