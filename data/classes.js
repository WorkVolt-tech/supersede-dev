...
export function onActiveSkill(skillId, player, statusEffects, enemy) {
  ...
  // ── Unwritten actives ────────────────────────────────────────────────
    ...
    // PLACEHOLDER: real implementation requires snapshot/restore of combat state.
    // For now: refund last 3 enemy damage events + 3 Absence stacks.
    // TODO: Real rewind requires saving statusEffects + HP + enemyHp at intervals.
    const refund = (statusEffects.cls_lastDamageTaken || 0) * 2  // approx 3-turn worth
    statusEffects.cls_burryHourHeal = refund
    statusEffects.cls_absenceStacks = (statusEffects.cls_absenceStacks || 0) + 3
    messages.push(`✎ Bury the Hour — the past unwinds. +${refund} HP, +3 Absence.`)
    ...
    // PLACEHOLDER: real implementation requires enemyAI.js to read a deletion list.
    // For now: apply silence (engine reads as no skills/abilities, just basic strikes).
    // TODO: Real version flags specific enemy skills as deleted.
    statusEffects.cls_silenceTurns = Math.max(statusEffects.cls_silenceTurns || 0, 99)
    statusEffects.cls_allWrittenOut = true
    messages.push('✎ All Written Out — the enemy can only swing now. Their record is bare.')
  }
...
export function onCombatStart(player, statusEffects) {
  ...
  // Oathbreaker — auto-swear oath at combat start based on primary branch
  // Defaults to first oath unlocked. Player can override via UI later (Path B
  // of design discussion deferred).
  if (player.active_class === 'oathbreaker') {
    if (hasSkill(player, 'oathbreaker_t1a')) {
      statusEffects.cls_currentOath = 'fury'
      messages.push('⚔ Oath of Fury sworn. +20% damage, -10% DEF.')
    } else if (hasSkill(player, 'oathbreaker_t1b')) {
      statusEffects.cls_currentOath = 'wall'
      messages.push('⚔ Oath of the Wall sworn. +25% DEF, -15% damage.')
    } else if (hasSkill(player, 'oathbreaker_t1c')) {
      statusEffects.cls_currentOath = 'hunt'
...
export function onPlayerAttackPost(player, statusEffects, enemy, damageDealt, wasCrit) {
  ...
  if (ac === 'unwritten') {
    ...
    // Empty Page (t3c): 10% chance per strike to delete enemy skill
    // PLACEHOLDER: real implementation requires enemyAI.js to read the
    // deletion list. For now, set a flag — Cold Page (4c) reads it for
    // damage bonus, and we apply enemyStunTurns +1 as the "deleted skill"
    // proxy effect (their next action gets skipped instead of denied).
    // TODO: Real version flags specific enemy skills as deleted.
    if (hasSkill(player, 'unwritten_t3c') && Math.random() < 0.10) {
      statusEffects.cls_emptyPageProc = true
      statusEffects.enemyStunTurns = Math.max(statusEffects.enemyStunTurns || 0, 1)
      messages.push('✎ Empty Page — a skill is forgotten.')
    }
  }
